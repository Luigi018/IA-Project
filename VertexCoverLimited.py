...
def evaluate_solution(cover, weights, cost_counter):
    cost_counter[0] += 1
...
def tabu_search_optimized(n, weights, matrix, max_iter=200, tabu_tenure=8, max_no_improvement=5, max_cost_evals=20000):
    ...
    for iteration in range(max_iter):
        ...
        for neighbor in neighbors:
            ...
            if neighbor_tuple not in tabu_list and neighbor_tuple not in visited_solutions:
                cost = evaluate_solution(neighbor, weights, cost_counter)
                
                if cost_counter[0] >= max_cost_evals:
                    print("Cost evaluation limit reached, stopping early.")
                    end_time = time.time()
                    return best_solution, best_cost, iteration_count, end_time - start_time, cost_counter[0]
    ...
    return best_solution, best_cost, iteration_count, elapsed_time, cost_counter[0]
...
if __name__ == "__main__":
    ...
    best_solution, best_cost, iteration_count, elapsed_time, cost_evals = tabu_search_optimized(n, weights, matrix)
    
    print("Best Solution:", best_solution)
    print("Best Cost:", best_cost)
    print("Total Iterations:", iteration_count)
    print(f"Algorithm Execution Time: {elapsed_time:.4f} seconds")
    print("Total Cost Evaluations:", cost_evals)
